...
const ProductCard = ({ product, onQuickView }) => {
  ...
  const primaryImage = product.images && product.images.length > 0 ? product.images[0] : '/images/products/placeholder.webp';
  ...
  return (
    <div className="group bg-white rounded-2xl border border-gray-100 p-3 shadow-sm hover:shadow-md transition-all duration-300 flex flex-col justify-between relative">
      ...
      <div className="relative aspect-square w-full rounded-xl bg-gray-50 overflow-hidden mb-3 border border-gray-50 flex items-center justify-center">
        ...
        {/* Discount Badge */}
        {discountPercent > 0 && (
          <span className="absolute top-2 left-2 z-10 px-2 py-0.5 bg-red-600 text-white text-[10px] font-bold rounded-md shadow-xs">
            -{discountPercent}%
          </span>
        )}
        ...
        <div className="absolute top-2 right-2 z-10 flex flex-col space-y-1.5 opacity-90 group-hover:opacity-100 transition-opacity">
          ...
          <button
            ...
            className="p-1.5 bg-white text-gray-400 hover:text-gray-700 rounded-full shadow-xs border border-gray-100 transition-colors"
            title="Compare"
          >
      ...
      <div className="px-1 flex-1 flex flex-col justify-between space-y-2">
        ...
        <button
          ...
          className="w-full mt-2 py-2 px-3 bg-red-600 hover:bg-red-700 text-white rounded-lg font-bold text-xs transition-colors flex items-center justify-center space-x-1 shadow-xs active:scale-[0.98]"
        >
